...
Now suppose Steve has been chosen at random from a group of people. The question is: Which is more likely?
...
For now, we will leave Steve’s story unresolved. We will come back to it at the end of the chapter, once we have developed the tools needed to think about this question more carefully.
...
This is the beginning of Bayes' theorem. For now, we will call it **Baby Bayes**. It says:
...
Now the hypothesis is no longer just true or false. It is a parameter that can take many possible values. We move from the likelihood of the data under a parameter value,
...
The notation changes slightly because $\theta$ may be continuous. If $\theta$ can take any value between 0 and 1, then we use a density, written with a lowercase (p), rather than a probability mass.

But the idea is the same. The prior describes our uncertainty about the parameter before seeing the data. The likelihood describes how well each parameter value predicts the data. The posterior describes our uncertainty about the parameter after seeing the data.
...
The beta distribution is a family of distributions on the interval from 0 to 1. By changing the values of $\alpha$ and $\beta$, we can create many different shapes.
...
beta_priors <- tibble(
  ...
  mutate(
    `Beta(1, 1)` = dbeta(theta, shape1 = 1, shape2 = 1),
    `Beta(2, 2)` = dbeta(theta, shape1 = 2, shape2 = 2),
    `Beta(8, 2)` = dbeta(theta, shape1 = 8, shape2 = 2),
    `Beta(2, 8)` = dbeta(theta, shape1 = 2, shape2 = 8)
  ) |>
  pivot_longer(
...
ggplot(beta_priors, aes(x = theta, y = density, linetype = Prior, color = Prior)) +
  geom_line(linewidth = 0.8) +
  labs(
    x = expression(theta),
    y = "Density"
...
In the late-night bus example, we used a beta prior for an unknown proportion:
...
The beta distribution and the binomial distribution work especially well together. When we combine a beta prior with a binomial likelihood, the posterior is also a beta distribution. This is called **conjugacy**.

A prior is called **conjugate** to a likelihood when the posterior belongs to the same family of distributions as the prior.
...
For a binomial problem, we can think of $\alpha$ as prior successes and $\beta$ as prior failures. This interpretation is not always exact, but it is often a useful way to build intuition.
...
> The posterior is formed by adding the observed successes and failures to the prior.
...
beta_update_plot <- tibble(
  theta = seq(0.001, 0.999, length.out = 1000)
) |>
  mutate(
    `Prior: Beta(1,1)` = dbeta(theta, shape1 = 1, shape2 = 1),
    `Posterior: Beta(27,15)` = dbeta(theta, shape1 = 27, shape2 = 15)
  ) |>
  pivot_longer(
    cols = c(`Prior: Beta(1,1)`, `Posterior: Beta(27,15)`),
    names_to = "Distribution",
    values_to = "density"
  )

ggplot(beta_update_plot, aes(x = theta, y = density, linetype = Distribution, color = Distribution)) +
  geom_line(linewidth = 0.8) +
...
The result is useful because it gives a full distribution over the possible values of $\theta$, not just a single estimate. Values of $\theta$ near the observed proportion receive more posterior weight, but there is still uncertainty.
...
beta_update_table <- tibble(
  ...
  `Interpretation` = c(
    "Start with a flat prior over possible values of $\\theta$",
    "Observe 26 yes responses and 14 no responses",
...
Conjugacy is convenient because it gives us a neat update rule. We do not need simulation, numerical integration, or complicated computation to find the posterior.

But it is important not to overstate the idea. Conjugacy is not what makes an analysis Bayesian. The Bayesian idea is the combination of a prior and a likelihood to produce a posterior. Conjugacy is just a mathematical shortcut that makes this combination especially simple.

In many real problems, the posterior does not have a neat formula. In those cases, Bayesian inference can still be done, but we need computational methods. For this chapter, the beta-binomial case gives us a simple way to see the main idea clearly.
...
prior_sensitivity |>
  transmute(
    ...
    `$P(\\theta > 0.50 \\mid X=26)$` = posterior_prob_majority
  ) |>
  gt() |>
  fmt_markdown(columns = everything()) |>
  fmt_number(
...
large_prior_sensitivity |>
  transmute(
    ...
    `$P(\\theta > 0.50 \\mid X=260)$` = posterior_prob_majority
  ) |>
  gt() |>
  fmt_markdown(columns = everything()) |>
  fmt_number(
...
Suppose, for illustration, that Steve was chosen from a group where 10% of people are librarians and 90% are farmers:
...
So the description is evidence for Steve being a librarian. It is three times more likely under the librarian hypothesis than under the farmer hypothesis:
...
So, under these assumptions, the probability that Steve is a librarian is 25%.
...
Steve's story is useful because it shows how easily our intuition can focus on resemblance and forget base rates. Bayes' theorem gives us a way to slow down.
...
Return to the Steve-style reasoning from the end of the chapter, but use a new example.
