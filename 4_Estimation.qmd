...
```{r, echo=FALSE}
#| classes: .enlarge-image
knitr::include_graphics("images/chapter4/income.jpg")
```
</center>
...
tibble(
  `Target question` = c(
    "What is the true mean annual income of all Australian households?",
    "What proportion of Australian households have annual income below $60,000?"
  ),
  `Population parameter` = c(
    "$\\mu$ = population mean",
    "$p$ = population proportion"
  ),
  `Sample statistic / estimate` = c(
    "$\\bar{x}$ = sample mean",
    "$\\hat{p}$ = sample proportion"
  ),
  `Value in illustrative sample` = c(
    "$\\bar{x} = 119{,}100$",
    "$\\hat{p} = 0.10$"
  ),
  `Interpretation` = c(
    "$119,100 is our estimate of the unknown population mean.",
    "10% is our estimate of the unknown population proportion below $60,000."
  )
) |>
  gt() |>
...
```{=html}
<iframe 
  src="interactive/sampling_distribution_app_animated_density_spaced.html"
  width="100%" 
  height="740"
  style="border: none;"
></iframe>
...
lower_crit <- qt(alpha / 2, df = df)
upper_crit <- qt(1 - alpha / 2, df = df)
...
left_tail <- subset(t_data, x <= lower_crit)
right_tail <- subset(t_data, x >= upper_crit)
...
ggplot(t_data, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = lower_crit, linetype = "dashed") +
  geom_vline(xintercept = upper_crit, linetype = "dashed") +
  
  annotate(
    "text",
    x = lower_crit,
    y = 0.08,
    label = round(lower_crit, 3),
    hjust = 1.1
  ) +
  ...
  annotate(
    ...
    x = upper_crit,
    y = 0.08,
    label = round(upper_crit, 3),
    hjust = -0.1
  ) +
  
  labs(
    title = "Two-Tailed t Distribution with 9 Degrees of Freedom",
    subtitle = "Shaded area = 0.05 total, with 0.025 in each tail",
    x = "t",
    y = "Density"
  ) +
  theme_minimal()
...
Suppose we collect the following data. There are 20 household surveyed, 10 from houses in the capital city, and 10 from houses from regional:
...
The two groups both contain a range of incomes. Both are right-skewed, with a few high-income households. The capital city sample contains one especially high income, which will pull its mean upward. Let us summarise the two groups.
...
But the phrase “in this sample” matters. We are not only interested in these 20 households. We are using these sample means to estimate a difference in the wider population. A difference in sample means is still a statistic. It varies from sample to sample. If we had sampled a different set of capital city households and a different set of regional households, the two sample means would change. Their difference would change too.
...
1. We choose a parameter. 
2. We calculate a statistic. 
3. We estimate the wobble using a standard error. 
4. We report uncertainty using an interval.

This is estimation.

## Coding the ideas in R

Throughout this chapter, we have used household income to think about estimation. We started with a small sample, calculated point estimates, thought about how those estimates would change from sample to sample, and then used standard errors and confidence intervals to describe uncertainty.

We can code the same ideas in R.

### Creating the household income sample

We begin by recreating the small household income sample used throughout the chapter.

```{r}
#| label: r-income-sample
...
This is the sample. It is not the population. The statistical question is how we can use these ten observed incomes to estimate something about a larger group of households.

### Calculating point estimates

A point estimate is a single number calculated from a sample and used to estimate an unknown population parameter.
...
In R, we can calculate these using `mean()`, `median()`, and a logical condition.

```{r}
mean_income <- mean(income_example$income)
median_income <- median(income_example$income)
prop_below_60000 <- mean(income_example$income < 60000)
...
The expression `income_example$income` means: use the `income` column from the `income_example` data frame.

The line:

```{r}
#| label: r-proportion-logic

income_example$income < 60000
```

checks whether each household income is below 60000. R returns `TRUE` or `FALSE` for each household. When we take the `mean()` of a logical vector, R treats `TRUE` as 1 and `FALSE` as 0. So:

```{r}
#| label: r-proportion-below-threshold

mean(income_example$income < 60000)
```

calculates the proportion of households in the sample with income below 60000.

We can put the estimates into one table.

```{r}
#| label: tbl-r-point-estimates
#| tbl-cap: "Point estimates from the household income sample"
#| echo: false

income_estimates <- tibble(
  quantity = c(
    "Mean income",
    "Median income",
    "Proportion below $60,000"
...
income_estimates |>
  mutate(
    estimate = c(
      dollar(mean_income, prefix = "$", accuracy = 1),
      dollar(median_income, prefix = "$", accuracy = 1),
      percent(prop_below_60000, accuracy = 1)
    )
  ) |>
  gt() |>
  cols_label(
...
The mean and median are both calculated from the same data, but they answer different questions. The mean is pulled upward by the high-income household. The median is closer to the middle of the observed incomes.

### Simulating repeated samples

In the chapter, we argued that an estimate could have turned out differently if we had drawn a different sample. We can use simulation to make this visible.

First, we create a large hypothetical population of household incomes. The values are simulated, not official ABS values. We use a right-skewed distribution because income data often have a long right tail.

```{r}
#| label: r-hypothetical-income-population

set.seed(24205242)

population_income <- tibble(
  income = round(
    rlnorm(
      n = 100000,
      meanlog = log(85000),
...
The function `rlnorm()` generates random values from a log-normal distribution. A log-normal distribution is useful here because it produces positive values and can be right-skewed.

In this code:

- `n = 100000` creates 100,000 household incomes;
- `meanlog = log(85000)` helps set the centre of the distribution;
- `sdlog = 0.55` controls the spread and skewness;
- `round(..., -2)` rounds values to the nearest 100.

We can plot this hypothetical population.

```{r}
#| label: fig-r-income-population
#| fig-cap: "A simulated right-skewed population of household incomes"
...
```{r}
#| label: r-one-sample-income

one_sample <- population_income |>
  slice_sample(n = 10)

one_sample
```

The function `slice_sample()` randomly selects rows from a data frame. Here it selects 10 households.

We can calculate the sample mean for this one sample.

```{r}
#| label: r-one-sample-mean

one_sample |>
  summarise(sample_mean = mean(income))
```

This gives one estimate. But if we take another sample, we will usually get another estimate.

### Building a sampling distribution

A sampling distribution describes how a statistic varies over repeated samples. We can approximate a sampling distribution by repeatedly sampling from our simulated population.

The code below takes 3000 samples. Each sample contains 10 households. For each sample, we calculate the sample mean.

```{r}
#| label: r-sampling-distribution
...
The function `map_dbl()` repeats the same calculation many times and stores the result as a numeric vector. In this example, it repeats the process of drawing a sample and calculating its mean.

Inside `map_dbl()`:

- `slice_sample(n = 10)` draws a random sample of 10 households;
- `summarise(mean_income = mean(income))` calculates the sample mean;
- `pull(mean_income)` extracts the result as a number.

Now we can plot the sampling distribution.

```{r}
#| label: fig-r-sampling-distribution
#| fig-cap: "A simulated sampling distribution of the sample mean for samples of size 10"
...
Each value in this plot is a sample mean from a sample of 10 households. This is different from a histogram of individual household incomes. It is a distribution of estimates.

We can compare the distribution of individual incomes with the distribution of sample means.

```{r}
#| label: fig-r-data-vs-sampling-distribution
#| fig-cap: "The data distribution and the sampling distribution are different objects"

library(patchwork)

p_data <- population_income |>
  slice_sample(n = 5000) |>
  ggplot(aes(x = income)) +
  ...
  labs(
    x = "Household income",
    y = "Count",
    title = "Individual household incomes"
  ) +
  theme_minimal()

p_means <- sample_means |>
  ggplot(aes(x = sample_mean)) +
  geom_histogram(bins = 35) +
  scale_x_continuous(
    labels = dollar_format(prefix = "$"),
...
The first plot is about households. The second plot is about estimates. This distinction is one of the most important ideas in the chapter.

### Standard errors and sample size

The standard error describes how much an estimate tends to vary from sample to sample.
...
income_summary <- income_example |>
  summarise(
    n = n(),
    mean_income = mean(income),
    sd_income = sd(income),
    se_mean = sd_income / sqrt(n)
  )

income_summary
```

The function `sd()` calculates the sample standard deviation. The function `sqrt()` calculates the square root.

The standard deviation describes variation among individual household incomes. The standard error describes variation among sample means.

We can also see how the standard error changes as the sample size changes. To isolate the effect of sample size, we hold the standard deviation fixed at the value from the original sample.

```{r}
#| label: tbl-r-standard-error-sample-size
#| tbl-cap: "Estimated standard error for different sample sizes"
#| echo: false

assumed_sd <- sd(income_example$income)

se_by_n <- tibble(
  sample_size = c(10, 25, 100, 400, 1600),
  standard_error = assumed_sd / sqrt(sample_size)
)
...
As the sample size increases, the standard error decreases. But it decreases with the square root of the sample size, not in a straight line.

```{r}
#| label: fig-r-standard-error-curve
#| fig-cap: "The standard error decreases as sample size increases, but with diminishing returns"

se_curve <- tibble(
  sample_size = seq(10, 2000, by = 10),
  standard_error = assumed_sd / sqrt(sample_size)
)

se_curve |>
  ggplot(aes(x = sample_size, y = standard_error)) +
  geom_line(linewidth = 1) +
  scale_y_continuous(labels = dollar_format(prefix = "$")) +
  labs(
    x = "Sample size",
...
To halve the standard error, we usually need about four times as much data. This is why precision can be expensive.
...
For a population mean, using the sample standard deviation, the interval is:
...
```{r}
#| label: r-confidence-interval-manual

n_income <- nrow(income_example)
mean_income <- mean(income_example$income)
sd_income <- sd(income_example$income)
se_income <- sd_income / sqrt(n_income)
t_multiplier <- qt(0.975, df = n_income - 1)

ci_lower <- mean_income - t_multiplier * se_income
ci_upper <- mean_income + t_multiplier * se_income

manual_ci <- tibble(
  mean_income = mean_income,
  standard_error = se_income,
...
The function `qt()` gives a quantile from the t-distribution. Here:

- `0.975` is used for a two-sided 95% confidence interval;
- `df = n_income - 1` gives the degrees of freedom.

We can display the result more cleanly.

```{r}
#| label: tbl-r-confidence-interval-manual
#| tbl-cap: "Manual 95% confidence interval for mean household income"
#| echo: false

manual_ci |>
  gt() |>
  fmt_currency(
    columns = c(mean_income, standard_error, lower, upper),
    currency = "AUD",
    decimals = 0
  ) |>
  fmt_number(
    columns = t_multiplier,
    decimals = 3
  ) |>
  cols_label(
    mean_income = "Sample mean",
    standard_error = "Standard error",
    t_multiplier = "t multiplier",
    lower = "Lower bound",
    upper = "Upper bound"
  )
```

R can also calculate this interval using `t.test()`.

```{r}
#| label: r-confidence-interval-ttest

t.test(income_example$income)
```

The function `t.test()` returns several pieces of information. For now, focus on the sample mean and the confidence interval. Later in the unit, we will return to the parts of this output related to hypothesis testing.

For this chapter, the key point is that the confidence interval gives a range of plausible values for the population mean, under the assumptions of the method.

### Simulating confidence intervals

A 95% confidence interval does not mean there is a 95% probability that this one realised interval contains the true mean. Instead, it describes the long-run performance of the method.

We can simulate this idea.

First, suppose there is a known population with a true mean. In real life, we usually do not know the true mean. But in a simulation, we can calculate it.

```{r}
#| label: r-true-population-mean

true_mean <- mean(population_income$income)
true_mean
```

Now we repeatedly draw samples of size 40. For each sample, we calculate a 95% confidence interval and record whether it contains the true mean.

```{r}
#| label: r-simulate-confidence-intervals
...
ci_simulation <- tibble(
  ...
  mutate(
    ...
    sample_mean = map_dbl(sample_data, ~ mean(.x$income)),
    sample_sd = map_dbl(sample_data, ~ sd(.x$income)),
    standard_error = sample_sd / sqrt(sample_size),
    multiplier = qt(0.975, df = sample_size - 1),
    lower = sample_mean - multiplier * standard_error,
    upper = sample_mean + multiplier * standard_error,
    contains_true_mean = lower <= true_mean & upper >= true_mean
  )

ci_simulation
```

The object `sample_data` is a list-column. Each row contains one sampled dataset. The functions `map_dbl()` then calculate summaries from each sampled dataset.

We can check how many intervals captured the true mean.

```{r}
#| label: r-confidence-interval-capture-rate

ci_simulation |>
  summarise(
    intervals = n(),
    captured = sum(contains_true_mean),
    capture_rate = mean(contains_true_mean)
  )
```

With only 80 intervals, the capture rate will not be exactly 95%. But it should usually be close.

We can plot the intervals.

```{r}
#| label: fig-r-confidence-interval-simulation
#| fig-cap: "Repeated confidence intervals. Most contain the true mean, but some miss."
...
ci_simulation |>
  ...
  geom_segment(
    aes(
      ...
      xend = upper,
      yend = sample_id,
      colour = contains_true_mean
    ),
    linewidth = 0.8
  ) +
  geom_point(aes(x = sample_mean), size = 1.5) +
...
Each horizontal line is one confidence interval. The dashed vertical line is the true mean in the simulated population. Most intervals cross the true mean, but some do not. This is the repeated-sampling meaning of confidence.
...
This does not mean the original data become normal. The household incomes are still right-skewed. The theorem is about the sample mean.

We can simulate sample means for different sample sizes.

```{r}
#| label: r-central-limit-theorem

set.seed(242052)

clt_simulation <- bind_rows(
  tibble(
    sample_size = "n = 5",
    sample_mean = replicate(
      5000,
      mean(sample(population_income$income, size = 5))
    )
  ),
  tibble(
    sample_size = "n = 25",
    sample_mean = replicate(
      5000,
      mean(sample(population_income$income, size = 25))
    )
  ),
  tibble(
    sample_size = "n = 100",
    sample_mean = replicate(
      5000,
      mean(sample(population_income$income, size = 100))
    )
  )
)
...
The function `replicate()` repeats the same expression many times. Here it repeatedly samples incomes and calculates a sample mean.

Now we plot the sampling distributions.

```{r}
#| label: fig-r-central-limit-theorem
#| fig-cap: "As sample size increases, the sampling distribution of the sample mean becomes more concentrated and more bell-shaped"
...
The original income distribution remains right-skewed. But the distribution of sample means becomes smoother, narrower, and more bell-shaped as the sample size grows.

### Comparing two means in R

The chapter also compared mean income between two groups: capital city households and regional households.

We can recreate that example.

```{r}
#| label: r-income-regions
...
```{r}
#| label: r-income-regions-summary
...
The function `group_by(region)` tells R to calculate the summaries separately for each region.
...
```{r}
#| label: r-difference-in-means

region_means <- income_regions |>
  group_by(region) |>
  summarise(mean_income = mean(income), .groups = "drop")

mean_capital <- region_means |>
  filter(region == "Capital city") |>
  pull(mean_income)

mean_regional <- region_means |>
  filter(region == "Regional") |>
  pull(mean_income)

diff_means <- mean_capital - mean_regional
...
```{r}
#| label: r-difference-in-means-se

region_stats <- income_regions |>
  group_by(region) |>
  summarise(
    n = n(),
    sd_income = sd(income),
...
n_capital <- region_stats |>
  filter(region == "Capital city") |>
  pull(n)

n_regional <- region_stats |>
  filter(region == "Regional") |>
  pull(n)

sd_capital <- region_stats |>
  filter(region == "Capital city") |>
  pull(sd_income)

sd_regional <- region_stats |>
  filter(region == "Regional") |>
  pull(sd_income)

se_diff <- sqrt(sd_capital^2 / n_capital + sd_regional^2 / n_regional)
...
R can also calculate a confidence interval for the difference in means using `t.test()`.

```{r}
#| label: r-independent-two-sample-ttest

t.test(income ~ region, data = income_regions)
```

The formula `income ~ region` tells R to compare income across the two values of `region`. The output includes a confidence interval for the difference in group means. Later in the unit, we will return to the hypothesis test part of this output.

### Paired comparisons in R

Finally, we considered a paired comparison: the same households measured in two years.

```{r}
#| label: r-paired-income-data

income_paired <- tibble(
  household = paste("Household", LETTERS[1:8]),
  income_2025 = c(72000, 85000, 91000, 105000, 116000, 128000, 142000, 210000),
  income_2026 = c(76000, 87000, 98000, 109000, 121000, 133000, 146000, 225000)
)

income_paired
```

These are paired data because each 2025 value is linked to a 2026 value for the same household.

The main step is to calculate the within-household change.

```{r}
#| label: r-paired-income-differences

income_paired <- income_paired |>
  mutate(change = income_2026 - income_2025)

income_paired
```

Now the paired problem becomes a one-sample estimation problem. We estimate the mean change.

```{r}
#| label: r-paired-income-summary
...
```{r}
#| label: r-paired-income-ci-manual

paired_summary <- income_paired |>
  summarise(
    n = n(),
    mean_change = mean(change),
    sd_change = sd(change),
    se_change = sd_change / sqrt(n),
    multiplier = qt(0.975, df = n - 1),
    lower = mean_change - multiplier * se_change,
    upper = mean_change + multiplier * se_change
  )

paired_summary
```

Or we can use `t.test()` with `paired = TRUE`.

```{r}
#| label: r-paired-income-ttest

t.test(
  income_paired$income_2026,
  income_paired$income_2025,
  paired = TRUE
)
```

The argument `paired = TRUE` tells R that the observations are linked. This matters because the method uses within-household differences rather than treating the two years as unrelated samples.

The lesson is the same as in the chapter:

> The formula should follow the structure of the data.

For independent groups, compare group means. For paired data, first calculate differences within each pair, then estimate the mean difference.
